...
public class UsernameSystem : MonoBehaviour
{
    ...
    public void UpdateTextMeshTo(Transform camera, bool altCam)
    {
        ...
        textMesh.fontSize = altCam ? (int)CustomMath.SingleAxisDistance(camera.position.y, transform.position.y) * fontScaler / 2: (int)Vector3.Distance(camera.position, transform.position) * fontScaler;
    }
}
